...
function highlightAnswer(questionElement, isIncorrect) {
  ...
  answerItems.forEach(item => {
    if (isIncorrect) {
      item.style.backgroundColor = '#ffcccc'; // backrous en rouge si la reponse est incorrecte 
      item.style.backgroundColor = ''; // Reinitialisation de la couleur du  background lorsque la reponse est correcte 
    }
  });
}
...
function resetState() {
  ...
  answerItems.forEach(item => {
    item.style.backgroundColor = ''; // reinitialiser la couleur du backroud pour toute les reponse 
  });
}
